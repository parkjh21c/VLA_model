...
class VLAFineTunePolicy(nn.Module):
    def __init__(
        self,
        action_dim=7,
        state_dim=8,
...
class GenericVLADataset(Dataset):
    def __init__(
        self,
        dataset_id,
        split,
        ...
        required_keys = [image_key1, image_key2, state_key, action_key]
        if task_key is not None:
            required_keys.append(task_key)
        if instruction_key is not None:
            required_keys.append(instruction_key)

        missing_keys = [key for key in required_keys if key not in self.dataset.column_names]
        if missing_keys:
            raise KeyError(
    ...
    def _resolve_text(self, sample):
        if self.instruction_key is not None:
        ...
        if self.task_key is not None:
            ...
            if task_index in self.task_map:
                return self.task_map[task_index]

        if task_index is None:
            return self.prompt_template

        return self.prompt_template.format(task_index=task_index)
    ...
    def __getitem__(self, idx):
        ...
        text = self._resolve_text(sample)
...
def load_task_map(task_map_path):
    if task_map_path is None:
        return {}

    task_map_file = Path(task_map_path)
    with task_map_file.open("r", encoding="utf-8") as fp:
        raw_task_map = json.load(fp)

    return {int(key): value for key, value in raw_task_map.items()}


def split_by_episode(dataset, train_ratio=0.9, seed=42, episode_key="episode_index"):
    if episode_key not in dataset.dataset.column_names:
        raise KeyError(
            f"Dataset must contain '{episode_key}' for episode-based split. "
            f"Available columns: {dataset.dataset.column_names}"
        )

    episode_ids = sorted(set(dataset.dataset[episode_key]))
    ...
    train_indices = [
        idx
        for idx, episode_index in enumerate(dataset.dataset[episode_key])
...
def build_dataloaders(args):
    dataset = GenericVLADataset(
        ...
        task_map=load_task_map(args.task_map_path),
...
def filter_state_dict(checkpoint, module_names):
    return {
        key: value
...
def load_partial_checkpoint(model, checkpoint_path, load_modules, weights_only=False):
    checkpoint = torch.load(
        checkpoint_path,
        map_location="cpu",
        weights_only=weights_only,
    )
    filtered_state = filter_state_dict(checkpoint, load_modules)
    load_result = model.load_state_dict(filtered_state, strict=False)

    print(f"Loaded checkpoint: {checkpoint_path}")
    print(f"Requested modules: {load_modules}")
...
def freeze_modules(model, module_names):
    for module_name in module_names:
        ...
        if module is None:
            raise AttributeError(f"Model has no module named '{module_name}'")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generic VLA fine-tuning entrypoint.")
    ...
    parser.add_argument("--task-map-path", default=None)
    ...
    parser.add_argument(
        "--load-modules",
        nargs="+",
        default=["vision_encoder", "language_encoder"],
    )
    parser.add_argument(
        "--freeze-modules",
        nargs="+",
    ...
    parser.add_argument("--checkpoint-weights-only", action="store_true")
    parser.add_argument("--cache-root", default=".cache")
    parser.add_argument("--save-dir", default="checkpoints/generic_finetune")
...
def main():
    ...
    if args.checkpoint_path:
        load_partial_checkpoint(
            ...
            weights_only=args.checkpoint_weights_only,
        )

    if args.freeze_modules:
        freeze_modules(model, args.freeze_modules)
